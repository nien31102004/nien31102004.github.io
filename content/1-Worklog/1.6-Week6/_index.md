---
title: "Week 6 Worklog"
date: 2026-05-25
weight: 6
chapter: false
pre: " <b> 1.6. </b> "
---

### Week 6 Objectives:

* Attend seminars to broaden AWS knowledge, soft skills, and career orientation.
* Learn from Hackathon experiences and real-world product development.
* Improve confidence, communication, and time-management skills.

### Tasks carried out this week:

| Day | Task | Start Date | Completion Date | Reference Material |
| --- | --- | --- | --- | --- |
| 2 | Attend a session on AWS Cloud Quest and AWS learning, lab, and certification platforms. | 25/05/2026 | 25/05/2026 | <https://aws.amazon.com/training/digital/aws-cloud-quest/> |
| 3 | Learn how to prepare for Hackathons, develop ideas, collaborate, and present in English. | 26/05/2026 | 26/05/2026 | <https://aws.amazon.com/developer/community/events/> |
| 4 | Attend a confidence-building session; note methods for preparation, practice, and overcoming hesitation. | 27/05/2026 | 27/05/2026 |  |
| 5 | Study the development, operation, and promotion process of the Tử Vi Đại Việt website project. | 28/05/2026 | 28/05/2026 |  |
| 6 | Attend a session on procrastination; learn causes and methods such as task decomposition and clear goals. | 29/05/2026 | 29/05/2026 |  |
| 7 and Sun | Summarize lessons on technology, communication, presentation, teamwork, and time management. | 30/05/2026 | 31/05/2026 |  |

### Week 6 Achievements:

* Expanded understanding of AWS Cloud Quest and AWS learning paths.
* Learned practical lessons from Hackathons and product development.
* Recognized the importance of communication, presentation, and confidence.
* Developed work-management methods to reduce procrastination.

### End-of-week Evaluation:

* The main objectives of the week were completed as planned.
* The activities strengthened my theoretical knowledge and practical AWS implementation skills.
* Issues encountered during the work were documented and resolved through AWS documentation and discussions with the mentor and team members.
* I reviewed the created resources and consolidated the necessary material for the internship report.
