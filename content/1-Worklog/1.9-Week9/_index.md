---
title: "Week 9 Worklog"
date: 2026-06-15
weight: 9
chapter: false
pre: " <b> 1.9. </b> "
---

### Week 9 Objectives:

* Attend seminars on career development, mentorship, and networking.
* Explore system-design thinking for applications serving large user bases.
* Understand the role of security, resource preparation, and caching in performance optimization.

### Tasks carried out this week:

| Day | Task | Start Date | Completion Date | Reference Material |
| --- | --- | --- | --- | --- |
| 2 | Attend Career Development Through Mentorship and identify the value of mentors in career growth. | 15/06/2026 | 15/06/2026 |  |
| 3 | Learn how to build a professional network, connect with experts, and leverage technology communities. | 16/06/2026 | 16/06/2026 |  |
| 4 | Analyze real project examples and the importance of initiative and learning new technologies. | 17/06/2026 | 17/06/2026 |  |
| 5 | Attend a URL Shortener system-design session; study protection layers before the backend and resource preparation. | 18/06/2026 | 18/06/2026 | <https://aws.amazon.com/architecture/> |
| 6 | Study caching mechanisms and how they reduce database queries and improve response time. | 19/06/2026 | 19/06/2026 | <https://docs.aws.amazon.com/AmazonElastiCache/latest/dg/WhatIs.html> |
| 7 và CN | Summarize lessons on system design, scalability, soft skills, and personal career direction. | 20/06/2026 | 21/06/2026 |  |

### Week 9 Achievements:

* Understood the value of mentorship, networking, and real projects for IT students.
* Developed system-design thinking around backend protection and future scalability.
* Learned how caching reduces database load and improves performance.
* Strengthened career direction by combining technical and communication skills.

### End-of-week Evaluation:

* The main objectives of the week were completed as planned.
* The activities strengthened my theoretical knowledge and practical AWS implementation skills.
* Issues encountered during the work were documented and resolved through AWS documentation and discussions with the mentor and team members.
* I reviewed the created resources and consolidated the necessary material for the internship report.
