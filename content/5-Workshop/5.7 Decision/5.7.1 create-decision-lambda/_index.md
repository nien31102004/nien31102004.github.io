---
title : "Create the Decision Engine Lambda"
date : 2024-01-01
weight : 1
chapter : false
pre : " <b> 5.7.1 </b> "
...
The Decision Engine Lambda combines malware scan results and AI analysis results to determine the final status of each document.
...
In the **Code source** section, replace the contents of `lambda_function.py` with the project Decision Engine source code, and then choose **Deploy**.
```text
import json
import os
import traceback
    ...
    try:
        ...
        if error_code == "ConditionalCheckFailedException":
            return api_response(
                ...
                {
                    "message": (
                        "The document has already been processed or "
...
# =========================================================
# Event normalization and authorization
# =========================================================

def normalize_event(event: Any) -> dict:
    """
    Supports:
    1. Direct Lambda-to-Lambda events.
    2. API Gateway events with a JSON body.
    """
...
def get_decision_source(event: dict) -> str:
    ...
    if decision_source not in VALID_DECISION_SOURCES:
        raise ValueError(
            "decisionSource must be SYSTEM_RULES or ADMIN_REVIEW"
        )
...
def require_admin(event: dict) -> None:
    ...
    if ADMIN_GROUP not in groups:
        raise ForbiddenError(
            f"Only members of the Cognito group '{ADMIN_GROUP}' "
            "may perform an administrator review"
        )
...
def resolve_final_action(
    ...
    """
    Priority order:
    1. A GuardDuty malware detection always results in REJECT.
    2. An administrator may only ALLOW or REJECT a document awaiting review.
    3. The system reads systemRecommendedAction after AI analysis is completed.
    4. If GuardDuty or AI processing is incomplete, the result is REVIEW.
    """
...
def resolve_admin_action(
    ...
    if action not in VALID_ADMIN_ACTIONS:
        raise ValueError(
            "Administrator action must be ALLOW or REJECT"
        )
    ...
    if current_status not in {
        ...
        raise ValueError(
            ...
            "MANUAL_REVIEW or DECISION_PENDING state"
        )
    ...
    if (
        ...
        raise ValueError(
            "A document cannot be approved until "
            "GuardDuty confirms NO_THREATS_FOUND"
        )
...
def process_move(
    ...
    if is_already_completed(
        ...
        return {
            ...
            "message": "The document was processed previously",
            "s3Key": item.get("s3Key"),
...
def validate_allow_preconditions(item: dict) -> None:
    ...
    if malware_status != "NO_THREATS_FOUND":
        raise ValueError(
            "The document cannot be moved to clean because "
            "GuardDuty has not confirmed that it is safe"
        )
...
def process_review(
    ...
    if (
        ...
        return {
            ...
            "s3Key": source_key,
            "message": "The document is already awaiting review",
        }
...
def get_valid_source_key(item: dict) -> str:
    ...
    if not source_key:
        raise ValueError("Document does not have an s3Key")
    ...
    if not decoded_key.startswith(expected_prefix):
        raise ValueError(
            "The source object is not under "
...
def copy_object_and_verify(
    ...
    if not destination_etag:
        raise RuntimeError("S3 CopyObject did not return an ETag")
    ...
    if not verified_etag:
        raise RuntimeError("Unable to verify the destination object")

    if destination_etag != verified_etag:
        raise RuntimeError(
            "The destination object ETag does not match the CopyObject result"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "The destination object size does not match the source object"
        )
...
# =========================================================
# Move state and DynamoDB finalization
# =========================================================
...
def validate_internal_admin_review(event):
    ...
    if internal_invocation is not True:
        raise PermissionError(
            "ADMIN_REVIEW may only be invoked internally"
        )
...
In the **General configuration** section, configure:
...
The Lambda execution role requires the following primary permissions:
...
```
**During troubleshooting, broad permissions were temporarily granted to the quarantine prefix. For the final configuration, apply least-privilege permissions.**
The S3 permission scope should be limited to the following bucket:
...
The Lambda function supports two decision sources:

+ `SYSTEM_RULES`: An automatic decision based on GuardDuty and AI results.
+ `ADMIN_REVIEW`: A manual decision submitted by an administrator through the document review API.

{{% notice warning %}}
The `ADMIN_GROUP` value must exactly match the group name in Amazon Cognito. If the group is named `Admins`, do not configure the value as `admin`.
{{% /notice %}}

{{% notice note %}}
The Decision Engine allows a document to be moved to `clean/` only after GuardDuty returns `NO_THREATS_FOUND`.
{{% /notice %}}